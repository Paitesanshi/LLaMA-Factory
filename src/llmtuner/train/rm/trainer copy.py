...
class PointwiseTrainer(Trainer):
    r"""
    Inherits Trainer to compute point loss.
    """
    ...
    def compute_loss(
        ...
        labels=(inputs["labels"].squeeze(-1)-1)/4
        loss, logits, values = model(input_ids=inputs['input_ids'],attention_mask=inputs['attention_mask'], lables=labels,output_hidden_states=True, return_dict=True)
        ...
        #labels = inputs["scores"].detach().clone() if "labels" in inputs else None 
        #labels=(labels.squeeze(-1)-1)/4
        #loss=self.mse_loss(values,labels).float()
    ...
    def prediction_step(
        ...
        Subclass and override to inject custom behavior.
        """
        labels = inputs["scores"].detach().clone() if "labels" in inputs else None  # backup labels
